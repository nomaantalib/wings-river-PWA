...
export default function LoadingScreen() {
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute w-[380px] h-[380px] rounded-full pointer-events-none transition-opacity duration-[2000ms] ease-out"
        style={{
          background: 'radial-gradient(circle, rgba(212,175,55,0.2) 0%, transparent 70%)',
          top: '50%', left: '50%',
          transform: 'translate(-50%, -50%)',
          filter: 'blur(55px)',
          opacity: iconLoaded ? 1 : 0.1,
        }}
      />

      <div className="relative flex flex-col items-center z-10 space-y-4">
        {/* Single Brand Logo Icon — 2-Second Smooth Low to High Fade-In */}
        <div
          className="relative w-24 h-24 sm:w-28 sm:h-28 rounded-3xl overflow-hidden shadow-2xl transition-all duration-[2000ms] ease-out"
          style={{
            opacity: iconLoaded ? 1 : 0.08,
            transform: iconLoaded ? 'scale(1) translateY(0)' : 'scale(0.92) translateY(8px)',
        ...
        {/* Brand Name — 2-Second Smooth Fade-In */}
        <h1
          className="font-serif text-xl sm:text-2xl font-bold tracking-[0.25em] text-[#F8E7A1] uppercase transition-all duration-[2000ms] ease-out"
          style={{
            opacity: iconLoaded ? 1 : 0.08,
            transform: iconLoaded ? 'translateY(0)' : 'translateY(6px)',
